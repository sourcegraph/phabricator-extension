...
abstract class PhabricatorController extends AphrontController
{
    ...
    public function willBeginExecution()
    {
        ...
        id(new PhabricatorAuthSessionEngine())->willServeRequestForUser($user);
        if (PhabricatorEnv::getEnvConfig('darkconsole.enabled')) {
            $dark_console = PhabricatorDarkConsoleSetting::SETTINGKEY;
            if ($user->getUserSetting($dark_console) || PhabricatorEnv::getEnvConfig('darkconsole.always-on')) {
    ...
    public function willSendResponse(AphrontResponse $response)
    {
        ...
        if ($response instanceof AphrontDialogResponse) {
            ...
            if ($response instanceof AphrontRedirectResponse) {
                if ($request->isAjax() || $request->isQuicksand()) {
                    return id(new AphrontAjaxResponse())->setContent(array('redirect' => $response->getURI(), 'close' => $response->getCloseDialogBeforeRedirect()));
                }
            }
        }
        return $response;
    ...
    {
        return id(new PhabricatorHandleQuery())->setViewer($this->getRequest()->getUser())->withPHIDs($phids)->execute();
    }
    ...
    {
        return PhabricatorPolicyFilter::hasCapability($this->getRequest()->getUser(), $this->getCurrentApplication(), $capability);
    }
    protected function requireApplicationCapability($capability)
    {
        PhabricatorPolicyFilter::requireCapability($this->getRequest()->getUser(), $this->getCurrentApplication(), $capability);
    }
    protected function explainApplicationCapability($capability, $positive_message, $negative_message)
    {
        ...
        $message = phutil_tag('div', array('class' => 'policy-capability-explanation'), array($icon, javelin_tag('a', array('href' => $explain_uri, 'sigil' => 'workflow'), $message)));
    ...
    public function newDialog()
    {
        ...
        return id(new AphrontDialogView())->setUser($this->getRequest()->getUser())->setSubmitURI($submit_uri);
    }
    public function newPage()
    {
        $page = id(new PhabricatorStandardPageView())->setRequest($this->getRequest())->setController($this)->setDeviceReady(true);
        $application = $this->getCurrentApplication();
        if ($application) {
    ...
    {
        return id(new PHUIApplicationMenuView())->setViewer($this->getViewer());
    }
    public function newCurtainView($object = null)
    {
        $viewer = $this->getViewer();
        $action_id = celerity_generate_unique_node_id();
        $action_list = id(new PhabricatorActionListView())->setViewer($viewer)->setID($action_id);
        ...
        $curtain = id(new PHUICurtainView())->setViewer($viewer)->setActionList($action_list);
        if ($object) {
    ...
    private function requireLegalpadSignatures()
    {
        ...
        $legalpad_installed = PhabricatorApplication::isClassInstalledForViewer($legalpad_class, $viewer);
        if ($legalpad_installed) {
            $sign_docs = id(new LegalpadDocumentQuery())->setViewer($viewer)->withSignatureRequired(1)->needViewerSignatures(true)->setOrder('oldest')->execute();
        ...
        if (!$must_sign_docs) {
            ...
            $engine = id(new PhabricatorAuthSessionEngine())->signLegalpadDocuments($viewer, $sign_docs);
            return null;
        }
        $request = $this->getRequest();
        $request->setURIMap(array('id' => head($must_sign_docs)->getID()));
